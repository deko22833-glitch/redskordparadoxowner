...
function initializeDatabase() {
    db.serialize(() => {
        ...
        // Create default server and channel if they don't exist
        db.get('SELECT id FROM servers WHERE id = 1', [], (err, row) => {
            if (!row) {
                db.run('INSERT INTO servers (id, name, icon, owner_id) VALUES (1, "Главный сервер", "Г", 1)', [], (err) => {
                    if (!err) {
                        db.run('INSERT INTO channels (name, type, server_id) VALUES ("general", "text", 1)', (err) => {
...
const userDB = {
    create: (username, hashedPassword) => {
        return new Promise((resolve, reject) => {
            ...
            // Используем username@local как placeholder для email, так как поле обязательное в БД
            const placeholderEmail = `${username}@local`;
            db.run(sql, [username, placeholderEmail, hashedPassword], function(err) {
                if (err) reject(err);
...
// Channel operations
const channelDB = {
    findByName: (name, serverId = 1) => {
        return new Promise((resolve, reject) => {
            const sql = 'SELECT * FROM channels WHERE name = ? AND server_id = ?';
            db.get(sql, [name, serverId], (err, row) => {
                if (err) reject(err);
                else resolve(row);
            });
        });
