...
app.post('/api/register', async (req, res) => {
    try {
        ...
        const user = await userDB.create(username.trim(), hashedPassword);
...
io.use((socket, next) => {
    ...
    jwt.verify(token, JWT_SECRET, (err, decoded) => {
        ...
        socket.userUsername = decoded.username;
        next();
    });
});
...
io.on('connection', async (socket) => {
    ...
    socket.on('send-message', async (messageData) => {
        try {
            ...
            let channel = await channelDB.findByName(channelId, 1);
            if (!channel) {
                channel = await channelDB.create(channelId, 'text', 1);
            }
